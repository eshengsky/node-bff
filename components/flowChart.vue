      ...
      <template v-for="(item, index) in apiData.flowchart">
        ...
        <div v-else-if="item.type === 'services'" :key="item.key" class="service-group">
          <div class="service-wrap">
            <div v-for="service in item.services" :key="service.key" class="service" @click="showService(service, item.key)">
              <div class="service-name">{{ service.name }}</div>
              <div class="service-path"><span class="service-method" :class="`method-${service.method}`">{{ service.method }}</span>{{ service.path }}</div>
            </div>
...
import { respFnKey, defaultJsonFn, defaultCommonFn } from '~/common/variables';
  ...
  methods: {
    ...
    delService (service: IService) {
      ...
      if (serviceGroup.services.length === 1) {
        ...
        // eslint-disable-next-line vue/no-parsing-error
        msg = '<strong>注意：所在的接口组和它前面的一般处理函数也会被一并删除！</strong><br>确定要删除该接口吗？';
      }
